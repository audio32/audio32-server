...
fn jack_client(sender: Sender<(Vec<u8>, u32)>) {
    ...
    // new
    let mut times = vec![(0, 0, 0, 0); 1024 * 8];
    ...
    let process_callback = move |client: &jack::Client, ps: &jack::ProcessScope| -> jack::Control {
        ...
        let callback_late = (jack_time as i128 - cycle_times.current_usecs as i128) * 1000;
        ...
        let len = times.len();
        times[i as usize % len] = (ptp_start_time, ptp_start_time_frames, time, callback_late);
        if i % (len as u32) == 0 {
            println!("{:?}", times);
        }
        prev_time = time;
